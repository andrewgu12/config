...
const C_Cpp_OutputManager = require("./C_Cpp_OutputManager");
...
function activate() {
    ...
    let clientOptions = {
        ...
        initializationOptions: {
            ...
            loggingLevel: con.get("loggingLevel")
        }
    };
    util.setProgress(util.getProgressExecutableStarted());
    var multilineCommentRules = {
        onEnterRules: [
            {
                beforeText: /^\s*\/\*\*(?!\/)([^\*]|\*(?!\/))*$/,
                afterText: /^\s*\*\/$/,
    ...
    vscode.languages.setLanguageConfiguration('c', multilineCommentRules);
    vscode.languages.setLanguageConfiguration('cpp', multilineCommentRules);
    languageClient = new vscode_languageclient_1.LanguageClient('C/Cpp Language Server', serverOptions, clientOptions);
    C_Cpp_OutputManager.setupOutputHandlers(languageClient);
    ...
    for (var key in con) {
        if (con.inspect(key).defaultValue === undefined)
            continue;
        let val = con.get(key);
        ...
        if (previousCppSettings[key] !== con.inspect(key).defaultValue) {
            nonDefaultInitialCppSettings[key] = (key == "clang_format_path") ? "..." : String(val);
...
function handleSettingsChange() {
    ...
    for (var key in cppSettings) {
        if (cppSettings.inspect(key).defaultValue === undefined)
            continue;
        let val = cppSettings.get(key);
        if (val instanceof Object)
            continue;
        if (val !== previousCppSettings[key]) {
            previousCppSettings[key] = val;
